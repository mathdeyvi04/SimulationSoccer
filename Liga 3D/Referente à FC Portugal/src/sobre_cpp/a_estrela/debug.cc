...
void teste_de_funcao_principal_a_estrela(){
    ...
    std::cout << duration_cast<microseconds>(fim - inicio).count() << "us -> Com Valores de Inicializacao.\n";
    ...
    std::cout << duration_cast<microseconds>(fim - inicio).count() << "us -> Com Outros Valores.\n";

    printf("\ntamanho_do_caminho_final = %d", tamanho_do_caminho_final);
    printf("\nApresentando valores obtidos: ");
    for(
    	int i = 0;
    		i < tamanho_do_caminho_final;
    		i++
    ){

    	printf("\ncaminho_final[%d] = %f", i, caminho_final[i]);
    }
    printf("\n");
}
